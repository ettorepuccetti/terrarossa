...
export const reservationRouter = createTRPCRouter({
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!checkDuration(input.startDateTime, input.endDateTime)) {
        throw new TRPCClientError(
          "Server: Reservations needs to be at least 1 hour and not longer than 2 hours"
        );
      }
      ...
      if (input.startDateTime.getTime() > input.endDateTime.getTime()) {
        throw new TRPCClientError(
          "Server: Start date cannot be after end date"
        );
      }
      if (input.overwriteName && ctx.session.user.role !== UserRoles.ADMIN) {
        throw new TRPCClientError(
          "Server: you are not authorized to overwrite the name of the reservation"
        );
      }
      ...
      if (
        sixHoursBeforeStart.isBefore(Date.now()) &&
        ctx.session.user.role !== UserRoles.ADMIN
      ) {
        throw new TRPCClientError(
          "Server: Reservations cannot be deleted 6 hours before the start time"
        );
      }
  ...
  deleteOne: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // reservation cannot be deleted six hours before the start time, using dayJS to add 6 hours to the start time
      // exception: admin can delete reservations at any time
      const sixHoursBeforeStart = dayjs(reservationToDelete.startTime).add(
        -6,
        "hour"
      );
      if (
